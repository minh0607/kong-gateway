...
| **Usage** | Per-consumer traffic **broken down by model** (requests, 5xx, in/out bandwidth). A **Period** selector — *All-time · Today · Yesterday · Last 7 days · Custom…*: **All-time** uses the live Prometheus counters (cumulative since Kong restart); the **dated ranges** (and **Custom…**, a from→to date picker with the end day inclusive) are computed from the request log (`started_at` + request/response sizes) and cover the retained log (~10 MB rolling). **Filter by consumer** and **by model** (two dropdowns) narrow the table to one consumer, one model, or both — client-side; stats, both views and CSV respect it. **Group by consumer** (default) rolls all of a consumer's models into one row with a per-model breakdown; untick for the flat consumer×model view. CSV export. A **Grafana ↗** link (also a sidebar **Grafana** entry) opens the historical per-consumer dashboard (`http://<gateway>:3000/d/kong-overview`; override with `localStorage['grafana_url']`). |
